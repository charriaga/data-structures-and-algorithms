...
const howMuchPencil = (str) => {
  ...
  for (let i = 0; i < str.length; i++) {
    let newString = str.slice(0, (str.length - i));
...
const listFoods = (recipe) => {
  ...
  for (let i=0; i < recipe.ingredients.length; i++){
    const newArr = recipe.ingredients[i].indexOf(' ');
    result.push(recipe.ingredients[i].slice(newArr + 1));
  }
  return result;
};
