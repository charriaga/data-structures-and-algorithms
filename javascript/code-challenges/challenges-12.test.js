...
const divisibleByFiveTwoToThePower = (input) => {
  const newBigArr = input.map(val => {
    let newArr = val.filter(num => typeof num === 'number');
    newArr = newArr.filter(num => num % 5 !== 0);

    newArr = newArr.map(val => val * val);
    return newArr;
  });
  return newBigArr;
};
...
let findMaleAndFemale = (data) => {
  ...
  newData = newData.map(val => val.name);
...
let findShortest = (data) => {
  let shortest = {height : 5000};
  data.map(val => {
    if (parseInt(val.height, 10) < parseInt(shortest.height)) {
      shortest = val;
    }
  });
