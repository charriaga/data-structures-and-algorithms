...
// ChaptGPt was consulted for this function
const sortByChildren = (charArray) => {
  const numOfChildrenArr = charArray.children.sort((a,b) => a - b);
  for (let i=0; i<numOfChildrenArr.length; i++) {
    if (numOfChildrenArr[i].children.length === numOfChildrenArr[i+1].children.length && numOfChildrenArr[i].house > numOfChildrenArr[i+1].house) {
      let temp = numOfChildrenArr[i];
      numOfChildrenArr[i] = numOfChildrenArr[i+1];
      numOfChildrenArr[i+1] = temp;
    }
  }
  return numOfChildrenArr;
};
...
const citiesAtoJ = (arr) => {
  ...
  arr.map(val => {
    const regex = /(\b[A-J].)/ig;
    const trueOrFalose = regex.test(val);
    if (trueOrFalose) {
      newArr.push(val);
    }
  });
